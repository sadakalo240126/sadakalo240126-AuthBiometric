...
final readonly class DeviceInfo
{
    ...
    {
        $hintPlat = trim(self::readServer('HTTP_SEC_CH_UA_PLATFORM'), "\" ");
        $hintVer  = trim(self::readServer('HTTP_SEC_CH_UA_PLATFORM_VERSION'), "\" ");

        $uaVer = '';
        if (preg_match('/Android\s+([\d.]+)/i', $ua, $m) === 1) {
            $uaVer = $m[1];
        } elseif (preg_match('/(?:iPhone OS|CPU OS)\s+([\d_]+)/i', $ua, $m) === 1) {
            $uaVer = str_replace('_', '.', $m[1]);
        }

        $name = '';
        if ($hintPlat !== '' && strtolower($hintPlat) !== 'unknown') {
            $name = $hintPlat;
        } else {
            $map = [
        ...
        $ver = $uaVer !== '' ? $uaVer : $hintVer;
        if ($ver !== '' && stripos($name, $ver) === false) {
            return $name . ' ' . $ver;
        }
        return $name;
    }
    ...
    private static function detectDeviceModel(string $ua): string
    {
        ...
        if (preg_match('/android[^;]*;\s*(?:wv\s;)?\s*([^;)]+?)\s+build\//i', $ua, $m) === 1) {
            $model = trim($m[1]);
            if ($model !== '' && strtolower($model) !== 'k' && stripos($model, 'wv') !== 0) {
                return $model;
            }
        }
        if (preg_match('/;\s*([A-Z]{2,}[-_]?\d{2,}[A-Z0-9\-]*)\b/i', $ua, $m) === 1) {
